...
var PipSqueek = function(options) {
    ...
    this.client.on('001', this.serverConnected.bind(this));
    //this.client.on('433', this._nickname_in_use());
    this.client.on('PRIVMSG', this.messageReceived.bind(this));
};

require('util').inherits(PipSqueek, require('eventemitter2').EventEmitter2);

PipSqueek.prototype.serverConnected = function() {
    // set +B to identify us as a bot
    this.client.send('MODE', this.nick, '+B');
    this.client.send('JOIN', this.channel);
};

PipSqueek.prototype.messageReceived = function(prefix, recipient, text) {
...
PipSqueek.prototype.parseCommands = function(message) {
